...
def my_classification_report(y_true, y_pred, target_names, digits=4):
    ...
    # 初始化 TP, FP, FN ,第i个类别的真实样本总数,全0数组
    ...
    pred_total = np.zeros(n_classes, dtype=int)  # 新增：每一类被预测出来的总样本数


    # 遍历所有样本统计
    for true_label_id, pred_label_id  in zip(y_true, y_pred):
        support[true_label_id] += 1
        pred_total[pred_label_id] += 1
    ...
    for i in range(n_classes):
        precision[i] = tp[i] / (pred_total[i] + eps)
        recall[i] = tp[i] / (support[i] + eps)
